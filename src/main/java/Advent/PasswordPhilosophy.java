...
public class PasswordPhilosophy {
    public static void main(String[] args) {
        ...
        for (String s1 : split) {
            ...
            if (isPasswordValidByOccurrence(Integer.parseInt(split1[0]),
                    Integer.parseInt(split2[0]),
                    split3[0].toCharArray()[0],
                    split2[2])) count++;
            if (isPasswordValidByPosition(Integer.parseInt(split1[0]),
                    Integer.parseInt(split2[0]),
                    split3[0].toCharArray()[0],
                    split2[2])) secondCount++;
        }
        System.out.println(count);
        System.out.println(secondCount);
    }

    private static boolean isPasswordValidByOccurrence(int min, int max, char character, String password) {
        int freq = 0;
        for (char c : password.toCharArray()) if (character == c) freq++;
        return freq >= min && freq <= max;
    }

    private static boolean isPasswordValidByPosition(int min, int max, char character, String password) {
